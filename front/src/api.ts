import type { AuditRow, Campaign, Dashboard, Donation, ReconciliationRow, User } from './types';
...
const mapDonation = (donation: ApiDonation): Donation => ({
    ...
    campaignName: typeof donation.campaign?.name === 'string' ? donation.campaign.name : undefined,
    amount: fromCents(donation.amountCents),
    maskedReceiptContact: typeof donation.receiptContactMasked === 'string' ? donation.receiptContactMasked : undefined,
...
export const api = {
    ...
    donation: (id: string) => request<ApiDonation>(`/api/donations/${id}`).then(mapDonation),
    ...
    dashboard: async (campaignId: string) => {
        ...
        const dashboard = await request<ApiDashboard>(`/api/admin/dashboard${query}`);
        return mapDashboard(dashboard);
    },
    campaigns: () => request<ApiCampaign[]>('/api/admin/campaigns').then((campaigns) => campaigns.map(mapCampaign)),
    ...
    donations: (filters: DonationFilters) =>
        request<ApiDonation[]>(`/api/admin/donations?${donationQuery(filters)}`).then((donations) =>
            donations.map(mapDonation),
        ),
    exportUrl: (filters: DonationFilters) => apiUrl(`/api/admin/donations/export.csv?${donationQuery(filters)}`),
    exportPdfUrl: (filters: DonationFilters) => apiUrl(`/api/admin/donations/export.pdf?${donationQuery(filters)}`),
    reconciliation: () =>
        request<ApiReconciliationRow[]>('/api/admin/reconciliation').then((rows) => rows.map(mapReconciliation)),
    audit: () => request<ApiAuditRow[]>('/api/admin/audit').then((rows) => rows.map(mapAudit)),
};
