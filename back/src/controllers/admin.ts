import { campaignInputSchema, campaignPatchSchema, donationQuerySchema, loginSchema } from '../../shared/dto.js';
import { clearSessionCookie, hasRole, verifyPassword } from '../auth.js';
import { httpError, parseBody, requireOrigin, type Context } from '../http.js';
...
import { dashboard, listDonations, reconciliation } from '../services/donations.js';
...
export const handleAdmin = async (context: Context) => {
    ...
    if (method === 'DELETE' && campaignMatch) {
        ...
        await audit(context.requestId, 'campaign_deleted', 'campaign', campaign.id, user.id);
        return ok(campaign);
    }
    ...
    if (method === 'GET' && pathname === '/api/admin/donations') {
        const parsed = donationQuerySchema.safeParse(Object.fromEntries(context.url.searchParams));
