...
const listDonations = async (context: Context) => {
  const query = donationQuerySchema.parse(Object.fromEntries(context.url.searchParams));
...
const dashboard = async (context: Context) => {
  ...
  const donations = await prisma.donation.findMany({ where, include: { campaign: true } });
  const totalRaisedEur = donations
    .filter((donation) => donation.currency === 'EUR')
  ...
  const goalAmountCents = donations[0]?.campaign.goalAmountCents ?? 0;
...
const handleAdmin = async (context: Context) => {
  ...
  if (method === 'POST' && pathname === '/api/admin/campaigns') {
    ...
    await audit(context.requestId, 'campaign_created', 'campaign', campaign.id, user.id);
    return campaign;
  }
  ...
  if (method === 'PATCH' && campaignMatch) {
    ...
    await audit(context.requestId, 'campaign_updated', 'campaign', campaign.id, user.id);
    return campaign;
  }

  const archiveMatch = pathname.match(/^\/api\/admin\/campaigns\/([^/]+)\/archive$/);
  if (method === 'POST' && archiveMatch) {
    ...
    await audit(context.requestId, 'campaign_archived', 'campaign', campaign.id, user.id);
    return campaign;
  }

  if (method === 'GET' && pathname === '/api/admin/dashboard') return dashboard(context);
  if (method === 'GET' && pathname === '/api/admin/donations') return listDonations(context);
  if (method === 'GET' && pathname === '/api/admin/reconciliation') {
    await audit(context.requestId, 'reconciliation_viewed', 'mastercard_event', null, user.id);
...
const handle = async (context: Context) => {
  ...
  if (method === 'POST' && pathname === '/api/donations') return createDonation(context);
...
const server = http.createServer((request, response) => {
  ...
  handle(context)
    ...
    .catch((error: unknown) => {
      ...
      send(response, status, { error: message, requestId }, requestId);
    });
});
