...
export const createSession = async (prisma: PrismaClient, response: ServerResponse, userId: string) => {
...
export const getUserFromRequest = async (prisma: PrismaClient, request: IncomingMessage) => {
...
export const deleteSession = async (prisma: PrismaClient, request: IncomingMessage) => {
    const token = readCookie(request, SESSION_COOKIE);
    if (!token) return;
...
export const hasRole = (user: User, role: Role) => roleRank[user.role] >= roleRank[role];
